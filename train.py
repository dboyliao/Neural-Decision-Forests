...
def parse_arg():
    ...
    parser.add_argument(
        "-dataset", choices=["mnist", "adult", "letter", "yeast"], default="mnist"
    )
...
def train(model, optim, db, opt):
    for epoch in range(1, opt.epochs + 1):
        ...
        test_loader = torch.utils.data.DataLoader(
            db["eval"], batch_size=opt.batch_size, shuffle=True
        )
        with torch.no_grad():
            for data, target in test_loader:
                ...
                test_loss += F.nll_loss(
                    torch.log(output), target, size_average=False
...
def main():
    ...
    # GPU
    opt.cuda = torch.cuda.is_available() and opt.gpuid >= 0 or False
    if opt.gpuid >= 0:
